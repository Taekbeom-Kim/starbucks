...
class dataCollectionCls:
    def discussionData(self, csv_save, codes):
        if csv_save:
            ...
            codes = codes[1590:1591] # 종목 50개만 추출(네이버 크롤링 보안정책상)
            ...
            for code in tqdm(codes):
                ...
                for page in pages:
                    urls = f'http://www.paxnet.co.kr/tbbs/list?tbbsType=L&id={str(code)}&page={str(page)}'

                    res = req.get(urls, headers=headers)
                    soup = BeautifulSoup(res.text, 'html.parser')#tit_150357588015148 > a.best-title

                    aa = soup.select("#comm-list > li:nth-child(2) > div.date > span")
                    bb = soup.find_all(href=re.compile("javascript:bbsWrtView"))

                    for a, b in zip(aa, bb):
                        # dt = datetime.datetime.strptime(a.contents[0].split(' ')[0].replace('.','-'), '%Y-%m-%d')
                        # dt = a['data-date-format']

                        dt = parse(a['data-date-format'])
                        ...
                        # 본문의 내용 가져오기
                        res2 = req.get(f'http://www.paxnet.co.kr/tbbs/view?id={str(code)}&seq={str(num)}&page={str(page)}', headers=headers)
                        soup2 = BeautifulSoup(res2.text, 'html.parser')

                        # 데이터프레임에 행 추가
                        soup22 = soup2.select('#bbsWrtCntn p')
                        soup22_list = []
                        for i in soup22:
                            soup22_list.append(i.find_all(text=True))
                        print(soup22_list)

                        df.loc[len(df)] = [code, dt, title, soup22_list]
...
test = dataCollectionCls()
test.discussionData(True, test.codeData())